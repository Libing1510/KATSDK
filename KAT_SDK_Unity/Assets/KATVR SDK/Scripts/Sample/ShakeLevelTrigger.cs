...
public class ShakeLevelTrigger : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.tag == "Player")
        {
            KATVR.KATVR_Global.KDevice_Landform2.SHAKE_LEVEL = level;
            Debug.Log("enter ShakeLevelTrigger " + level);
    ...
    private void OnTriggerExit(Collider other)
    {
        if (other.tag == "Player")
        {
            Debug.Log("exit ShakeLevelTrigger " + level);
    ...
    private void Reset()
    {
        ...
        transform.name = "ShakeLevelTrigger";
        if (!ground)
        {
